 ...
 *  YOKTUR ama GEREKMEZ DE: theme-config zaten rastgele anahtarları kabul eder
 *  (B2B_Config::save → deep_merge, whitelist yok).
 *
 *  ÖNEMLİ SINIR: Bu veri kaydedilir ama web sitesinde GÖRÜNMESİ, sitenin
 *  temasının (b2b-core-theme-1) "showcase.featured_products" /
 *  "showcase.banners" alanlarını okuyup basacak şekilde güncellenmesine
 *  bağlıdır — bu panel yalnızca veriyi hazırlar ve gönderir.
 *
 *  "Kampanyalı Ürünler" adı sitede AYRICA WooCommerce indirim fiyatına
 *  (sale_price) göre otomatik süzülen bir görünüm için de kullanılıyor
 *  (?b2b_gorunum=kampanya). Bu ekrandaki liste onunla KARIŞTIRILMASIN diye
 *  bilinçli olarak ayrı bir anahtarda ("showcase.featured_products") tutulur.
 * ========================================================================*/

function vitrinDurumYaz(tur, baslik, mesaj) {
  const kutu = $('#vitrinDurum');
  if (!kutu) return;
...
/** Vitrin sekmesini açar/yeniler: mevcut showcase verisini (varsa) siteden çeker. */
async function vitrinSekmesiYukle(zorla) {
  const d = ekDurum();
  if (!d) return;

  if (d.vitrin.yukleniyor) return;
  ...
  try {
    ...
    if (ekDemoMu()) {
      ...
      if (!d.vitrin.yuklendi) {
        d.vitrin.featured = (d.urunler || []).slice(0, 3).map(function (u) {
          return { id: u.id, ad: u.ad, kod: u.kod, gorsel: u.gorsel };
        });
        d.vitrin.banners = [];
      }
    ...
    d.vitrin.banners = Array.isArray(showcase.banners)
      ? showcase.banners.map(function (b) {
          return {
...
/* ---------------- Öne çıkan / kampanyalı ürünler ---------------- */

function vitrinUrunListesiCiz() {
  const d = ekDurum();
  const kap = $('#vitrinUrunListesi');
  const sayac = $('#vitrinUrunSayisi');
  if (!d || !kap) return;

  if (sayac) sayac.textContent = d.vitrin.featured.length;

  if (!d.vitrin.featured.length) {
    kap.innerHTML = '<div class="text-base text-slate-400 dark:text-slate-500 italic py-4 text-center">' +
      'Henüz ürün eklenmedi. Yukarıdaki arama kutusundan ürün seçin.</div>';
    return;
  }

  kap.innerHTML = d.vitrin.featured.map(function (u, i) {
    const sonMu = i === d.vitrin.featured.length - 1;
    return '<div class="flex items-center gap-3 p-3 rounded-xl border-2 border-slate-200 dark:border-slate-700 ' +
                'bg-slate-50 dark:bg-slate-900" data-vitrin-urun="' + kacis(String(u.id)) + '">' +
      '<img src="' + kacis(u.gorsel || YEDEK_GORSEL) + '" alt="" ' +
           'class="w-12 h-12 object-cover rounded-lg border-2 border-slate-200 dark:border-slate-700 shrink-0" />' +
      '<div class="flex-1 min-w-0">' +
        '<div class="font-bold truncate">' + kacis(u.ad) + '</div>' +
...
/** Sıra düğmesi (▲/▼) — devre dışıysa tıklanamaz ve soluk görünür. */
function vitrinSiraDugmesiHtml(eylem, simge, devreDisiMi) {
  return '<button data-eylem="' + eylem + '" ' + (devreDisiMi ? 'disabled' : '') +
         ' class="w-9 h-9 rounded-lg border-2 border-slate-300 dark:border-slate-600 font-black transition ' +
         (devreDisiMi ? 'opacity-30 cursor-not-allowed' : 'hover:bg-slate-200 dark:hover:bg-slate-700') +
         '">' + simge + '</button>';
}

/** Arama kutusuna yazılırken eşleşen (henüz vitrinde olmayan) ürünleri listeler. */
function vitrinUrunAra(metin) {
  const d = ekDurum();
  const kap = $('#vitrinUrunAramaSonuc');
  if (!d || !kap) return;

  const q = String(metin || '').trim().toLocaleLowerCase('tr');

  if (!q) { kap.classList.add('hidden'); kap.innerHTML = ''; return; }

  const eklenmisIdler = {};
  d.vitrin.featured.forEach(function (u) { eklenmisIdler[String(u.id)] = true; });

  const sonuclar = (d.urunler || []).filter(function (u) {
    if (eklenmisIdler[String(u.id)]) return false;
    const ad = String(u.ad || '').toLocaleLowerCase('tr');
    const kod = String(u.kod || '').toLocaleLowerCase('tr');
    return ad.indexOf(q) !== -1 || kod.indexOf(q) !== -1;
  }).slice(0, 20);

  kap.classList.remove('hidden');

  if (!sonuclar.length) {
    kap.innerHTML = '<div class="p-3 text-base text-slate-400 text-center">Sonuç bulunamadı.</div>';
    return;
  }

  kap.innerHTML = sonuclar.map(function (u) {
    return '<button type="button" data-eylem="vitrin-urun-ekle" data-id="' + kacis(String(u.id)) + '" ' +
                  'class="flex items-center gap-3 p-2 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-700 ' +
                         'text-left w-full transition">' +
      '<img src="' + kacis(u.gorsel || YEDEK_GORSEL) + '" alt="" ' +
           'class="w-10 h-10 object-cover rounded-lg border-2 border-slate-200 dark:border-slate-700 shrink-0" />' +
      '<div class="flex-1 min-w-0">' +
        '<div class="font-bold truncate">' + kacis(u.ad) + '</div>' +
        '<div class="text-xs text-slate-500 dark:text-slate-400 truncate">Kod: ' + kacis(u.kod || '—') + '</div>' +
      '</div>' +
      '<span class="text-emerald-600 dark:text-emerald-400 font-black text-sm shrink-0">+ EKLE</span>' +
    '</button>';
  }).join('');
}

function vitrinUrunEkle(id) {
  const d = ekDurum();
  if (!d) return;
  if (d.vitrin.featured.some(function (u) { return String(u.id) === String(id); })) return;

  const u = (d.urunler || []).filter(function (x) { return String(x.id) === String(id); })[0];
  if (!u) return;

  d.vitrin.featured.push({ id: u.id, ad: u.ad, kod: u.kod, gorsel: u.gorsel });
  vitrinUrunListesiCiz();

  const arama = $('#vitrinUrunArama');
  const sonucKutu = $('#vitrinUrunAramaSonuc');
  if (arama) { arama.value = ''; arama.focus(); }
  if (sonucKutu) { sonucKutu.classList.add('hidden'); sonucKutu.innerHTML = ''; }
}

function vitrinUrunCikar(id) {
  const d = ekDurum();
  if (!d) return;
  d.vitrin.featured = d.vitrin.featured.filter(function (u) { return String(u.id) !== String(id); });
  vitrinUrunListesiCiz();
}

function vitrinUrunSiraDegistir(id, yon) {
  const d = ekDurum();
  if (!d) return;

  const liste = d.vitrin.featured;
  const i = liste.findIndex(function (u) { return String(u.id) === String(id); });
  const j = i + yon;
...
/* ---------------- Ana sayfa slider / banner ---------------- */

function vitrinBannerListesiCiz() {
  const d = ekDurum();
  const kap = $('#vitrinBannerListesi');
  if (!d || !kap) return;
  ...
  kap.innerHTML = d.vitrin.banners.map(function (b, i) {
    const sonMu = i === d.vitrin.banners.length - 1;
    return '<div class="rounded-xl border-2 border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 ' +
                'p-4 flex gap-3" data-banner-index="' + i + '">' +
      '<img src="' + kacis(b.gorselUrl || YEDEK_GORSEL) + '" alt="" ' +
           'class="w-28 h-20 object-cover rounded-lg border-2 border-slate-200 dark:border-slate-700 shrink-0" />' +
      '<div class="flex-1 min-w-0 flex flex-col gap-2">' +
        '<input data-banner-alan="baslik" type="text" placeholder="Başlık (opsiyonel)" value="' + kacis(b.baslik || '') + '" ' +
               'class="w-full h-11 px-3 rounded-lg text-base font-semibold bg-white dark:bg-slate-800 ' +
                      'border-2 border-slate-300 dark:border-slate-600 outline-none focus:border-marka-600" />' +
        '<input data-banner-alan="link" type="text" placeholder="Bağlantı (https://… veya /kategori/aletler)" value="' + kacis(b.link || '') + '" ' +
               'class="w-full h-11 px-3 rounded-lg text-base font-mono bg-white dark:bg-slate-800 ' +
                      'border-2 border-slate-300 dark:border-slate-600 outline-none focus:border-marka-600" />' +
      '</div>' +
...
/** Seçilen dosyayı banner olarak ekler (canlıda siteye yükler, demoda yalnızca önizler). */
async function vitrinBannerEkle(dosya) {
  const d = ekDurum();
  if (!d || !dosya) return;

  if (!/^image\//.test(dosya.type)) {
    bildir('Lütfen bir görsel dosyası seçin (PNG, JPG…).', 'uyari');
    return;
  }
  if (dosya.size > 5 * 1024 * 1024) {
    bildir('Banner görseli çok büyük (en fazla 5 MB).', 'uyari');
    return;
  }

  const btn = $('#vitrinBannerEkleBtn');
  const geriAl = btn ? butonuMesgulEt(btn, 'YÜKLENİYOR…') : function () {};

  try {
    const veriAdresi = await dosyayiVeriAdresineCevir(dosya);

    /* Demo/eklentisiz: yalnızca yerel önizleme, siteye yüklenmez. */
    if (ekDemoMu() || !ekEklentiVarMi()) {
      d.vitrin.banners.push({ gorselUrl: veriAdresi, mediaId: 0, baslik: '', link: '' });
      vitrinBannerListesiCiz();
      return;
    }
    ...
    const cevap = await b2b('media', {
      metod: 'POST',
      govde: { filename: dosya.name, data: veriAdresi, title: 'Banner' },
      sureAsimi: 45000
    });

    if (!cevap || !cevap.ok) {
      bildir('Banner yüklenemedi:\n' + ekHataMetni(cevap), 'hata');
      return;
    }

    d.vitrin.banners.push({
      gorselUrl: (cevap.veri && cevap.veri.url) || veriAdresi,
      mediaId: (cevap.veri && cevap.veri.id) || 0,
      baslik: '',
      link: ''
    });
    vitrinBannerListesiCiz();
  } catch (e) {
    bildir('Banner yüklenirken beklenmeyen bir hata oluştu:\n' + String((e && e.message) || e), 'hata');
  } finally {
    geriAl();
  }
}

function vitrinBannerSil(index) {
  const d = ekDurum();
  if (!d) return;
  d.vitrin.banners.splice(index, 1);
  vitrinBannerListesiCiz();
}

function vitrinBannerSiraDegistir(index, yon) {
  const d = ekDurum();
  if (!d) return;

  const liste = d.vitrin.banners;
  const j = index + yon;
  if (j < 0 || j >= liste.length) return;

  const gecici = liste[index];
  liste[index] = liste[j];
  liste[j] = gecici;
  vitrinBannerListesiCiz();
}

/** Banner başlık/link kutusu yazılırken listeyi YENİDEN ÇİZMEDEN veriyi günceller. */
function vitrinBannerAlanGuncelle(index, alan, deger) {
  const d = ekDurum();
  if (!d || !d.vitrin.banners[index]) return;
  d.vitrin.banners[index][alan] = deger;
}

/* ---------------- Siteye gönder ---------------- */

async function vitrinGonder() {
  const d = ekDurum();
  if (!d) return;
  ...
  const btn = $('#vitrinGonderBtn');
  const geriAl = btn ? butonuMesgulEt(btn, 'GÖNDERİLİYOR…') : function () {};

  try {
    const govde = {
      config: {
        showcase: {
          featured_products: d.vitrin.featured.map(function (u, i) {
...
function ekOlaylariBagla() {
   ...
   *  G2) WEB VİTRİNİ
   * ------------------------------------------------------------*/
  const vitrinYenileBtn = $('#vitrinYenileBtn');
  if (vitrinYenileBtn) {
    vitrinYenileBtn.addEventListener('click', function () { vitrinSekmesiYukle(true); });
  }

  const vitrinGonderBtn = $('#vitrinGonderBtn');
  if (vitrinGonderBtn) {
    vitrinGonderBtn.addEventListener('click', function () { vitrinGonder(); });
  }

  /* --- Ürün arama (yazarken bekleyip arasın) --- */
  const vitrinUrunAramaKutu = $('#vitrinUrunArama');
  if (vitrinUrunAramaKutu) {
    vitrinUrunAramaKutu.addEventListener('input', function () {
      const deger = vitrinUrunAramaKutu.value;
      clearTimeout(ekDurum().vitrinAramaZaman);
      ekDurum().vitrinAramaZaman = setTimeout(function () { vitrinUrunAra(deger); }, 250);
    });
    vitrinUrunAramaKutu.addEventListener('blur', function () {
      /* Sonuca tıklanabilsin diye küçük bir gecikmeyle kapat. */
      setTimeout(function () {
        const kutu = $('#vitrinUrunAramaSonuc');
        if (kutu) kutu.classList.add('hidden');
      }, 200);
    });
  }

  const vitrinUrunAramaSonuc = $('#vitrinUrunAramaSonuc');
  if (vitrinUrunAramaSonuc) {
    vitrinUrunAramaSonuc.addEventListener('mousedown', function (o) {
      /* mousedown: input'un blur'undan ÖNCE çalışır, tıklama kaybolmaz. */
      const btn = ekEnYakin(o.target, '[data-eylem="vitrin-urun-ekle"]');
      if (btn) { o.preventDefault(); vitrinUrunEkle(btn.dataset.id); }
    });
  }

  const vitrinUrunListesi = $('#vitrinUrunListesi');
  if (vitrinUrunListesi) {
    vitrinUrunListesi.addEventListener('click', function (o) {
      const satir = ekEnYakin(o.target, '[data-vitrin-urun]');
      if (!satir) return;
      const id = satir.dataset.vitrinUrun;

      if (ekEnYakin(o.target, '[data-eylem="vitrin-urun-yukari"]')) { vitrinUrunSiraDegistir(id, -1); return; }
      if (ekEnYakin(o.target, '[data-eylem="vitrin-urun-asagi"]')) { vitrinUrunSiraDegistir(id, 1); return; }
      if (ekEnYakin(o.target, '[data-eylem="vitrin-urun-cikar"]')) { vitrinUrunCikar(id); return; }
    });
  }

  /* --- Banner ekle --- */
  const vitrinBannerEkleBtn = $('#vitrinBannerEkleBtn');
  if (vitrinBannerEkleBtn) {
    vitrinBannerEkleBtn.addEventListener('click', function () { $('#vitrinBannerDosyaSec').click(); });
  }

  const vitrinBannerDosyaSec = $('#vitrinBannerDosyaSec');
  if (vitrinBannerDosyaSec) {
    vitrinBannerDosyaSec.addEventListener('change', function (o) {
      const dosya = o.target.files && o.target.files[0];
      vitrinBannerEkle(dosya);
      o.target.value = '';
    });
  }

  const vitrinBannerListesi = $('#vitrinBannerListesi');
  if (vitrinBannerListesi) {
    vitrinBannerListesi.addEventListener('click', function (o) {
      const satir = ekEnYakin(o.target, '[data-banner-index]');
      if (!satir) return;
      const index = Number(satir.dataset.bannerIndex);

      if (ekEnYakin(o.target, '[data-eylem="banner-yukari"]')) { vitrinBannerSiraDegistir(index, -1); return; }
      if (ekEnYakin(o.target, '[data-eylem="banner-asagi"]')) { vitrinBannerSiraDegistir(index, 1); return; }
      if (ekEnYakin(o.target, '[data-eylem="banner-sil"]')) { vitrinBannerSil(index); return; }
    });

    /* Başlık/link kutuları: yazarken YENİDEN ÇİZMEDEN veriyi güncelle. */
    vitrinBannerListesi.addEventListener('input', function (o) {
      const alanKutu = o.target.closest('[data-banner-alan]');
      const satir = ekEnYakin(o.target, '[data-banner-index]');
      if (!alanKutu || !satir) return;
      vitrinBannerAlanGuncelle(Number(satir.dataset.bannerIndex), alanKutu.dataset.bannerAlan, alanKutu.value);
    });
  }
...
function ekDurumBaslangici() {
  ...
  /* G2) Web Vitrini */
  d.vitrin = {
    featured: [],   // [{ id, ad, kod, gorsel }] — sıralı
    banners: [],    // [{ gorselUrl, mediaId, baslik, link }] — sıralı
    yuklendi: false,
    yukleniyor: false
  };
}
...
window.vitrinSekmesiYukle = vitrinSekmesiYukle;
window.vitrinUrunListesiCiz = vitrinUrunListesiCiz;
window.vitrinUrunAra = vitrinUrunAra;
window.vitrinUrunEkle = vitrinUrunEkle;
window.vitrinUrunCikar = vitrinUrunCikar;
window.vitrinUrunSiraDegistir = vitrinUrunSiraDegistir;
window.vitrinBannerListesiCiz = vitrinBannerListesiCiz;
window.vitrinBannerEkle = vitrinBannerEkle;
window.vitrinBannerSil = vitrinBannerSil;
window.vitrinBannerSiraDegistir = vitrinBannerSiraDegistir;
window.vitrinBannerAlanGuncelle = vitrinBannerAlanGuncelle;
window.vitrinGonder = vitrinGonder;
